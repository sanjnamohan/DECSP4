...
{
	pair<string,string> fn_k = get_filename_key("ABCDE");
	string filename = fn_k.first;
	string key = fn_k.second;

	KVStore *kvs = get_file_obj(filename);
	cout<<kvs->add_key(key,10)<<endl;
	cout<<kvs->get_key(key)<<endl;
	cout<<kvs->add_key(key,11)<<endl;
	cout<<kvs->get_key(key)<<endl;
	cout<<kvs->get_key_count()<<endl;

	fn_k = get_filename_key("AB123");
	filename = fn_k.first;
	key = fn_k.second;

	kvs = get_file_obj(filename);
	cout<<kvs->add_key(key,34)<<endl;
	cout<<kvs->get_key(key)<<endl;
	cout<<kvs->add_key(key,45)<<endl;
	cout<<kvs->get_key(key)<<endl;
	cout<<kvs->get_key_count()<<endl;
	cout<<kvs->del_key(key)<<endl;
